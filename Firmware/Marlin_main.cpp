  ...
  if ((boot_app_magic == BOOT_APP_MAGIC) && (boot_app_flags & BOOT_APP_FLG_USER0))
  {
    ...
    if (lang_get_header(lang, &header, &src_addr))
    {
      ...
      if ((state * LANGBOOT_BLOCKSIZE) < header.size)
      {
        ...
        if (size > LANGBOOT_BLOCKSIZE)
          size = LANGBOOT_BLOCKSIZE;
...
#undef LT_PRINT_TEST
  ...
  if (mcu & 1)
    puts_P(MSG_POWERUP);
  if (mcu & 2)
    puts_P(MSG_EXTERNAL_RESET);
  if (mcu & 4)
    puts_P(MSG_BROWNOUT_RESET);
  if (mcu & 8)
    puts_P(MSG_WATCHDOG_RESET);
  if (mcu & 32)
    puts_P(MSG_SOFTWARE_RESET);
  MCUSR = 0;
  ...
  if (!(hw_changed & 0b10))
  { //if printer version wasn't changed, check for eeprom version and retrieve settings from eeprom in case that version wasn't changed
    previous_settings_retrieved = Config_RetrieveSettings();
  }
  else
  { //printer version was changed so use default settings
    Config_ResetDefault();
  }
  ...
  if (tmc2130_wave_fac[X_AXIS] == 0xff)
    tmc2130_wave_fac[X_AXIS] = 0;
  if (tmc2130_wave_fac[Y_AXIS] == 0xff)
    tmc2130_wave_fac[Y_AXIS] = 0;
  if (tmc2130_wave_fac[Z_AXIS] == 0xff)
    tmc2130_wave_fac[Z_AXIS] = 0;
  if (tmc2130_wave_fac[E_AXIS] == 0xff)
  ...
  farm_mode = eeprom_read_byte((uint8_t *)EEPROM_FARM_MODE);
  EEPROM_read_B(EEPROM_FARM_NUMBER, &farm_no);
  if ((farm_mode == 0xFF && farm_no == 0) || (farm_no == static_cast<int>(0xFFFF)))
    farm_mode = false; //if farm_mode has not been stored to eeprom yet and farm number is set to zero or EEPROM is fresh, deactivate farm mode
  if (farm_no == static_cast<int>(0xFFFF))
    farm_no = 0;
  if (farm_mode)
  {
    prusa_statistics(8);
  ...
  { //bowden length used for SNMM
    int _z = BOWDEN_LENGTH;
    for (int i = 0; i < 4; i++)
      EEPROM_save_B(EEPROM_BOWDEN_LENGTH + i * 2, &_z);
  }
#endif
  ...
  if (tmc2130_home_origin[X_AXIS] == 0xff)
    tmc2130_home_origin[X_AXIS] = 0;
  if (tmc2130_home_bsteps[X_AXIS] == 0xff)
    tmc2130_home_bsteps[X_AXIS] = 48;
  if (tmc2130_home_fsteps[X_AXIS] == 0xff)
  ...
  if (tmc2130_home_origin[Y_AXIS] == 0xff)
    tmc2130_home_origin[Y_AXIS] = 0;
  if (tmc2130_home_bsteps[Y_AXIS] == 0xff)
    tmc2130_home_bsteps[Y_AXIS] = 48;
  if (tmc2130_home_fsteps[Y_AXIS] == 0xff)
    tmc2130_home_fsteps[Y_AXIS] = 48;

  tmc2130_home_enabled = eeprom_read_byte((uint8_t *)EEPROM_TMC2130_HOME_ENABLED);
  if (tmc2130_home_enabled == 0xff)
...
bool check_commands()
{
  ...
  while (buflen)
  {
    if ((code_seen("M84")) || (code_seen("M 84")))
...
{
#ifdef PSU_Delta
  if (start_high_power_section == true)
    enable_force_z();
#endif //PSU_Delta
  uint8_t silent;
    ...
    */
    ...
    case 1: // G1
      if (Stopped == false)
      {
        ...
        { //protection against total_filament_used overflow
    ...
    case 76:
    {
      ...
      if (true)
      {
        ...
        if ((current_temperature_pinda > 35) && (farm_mode == false))
        {
          ...
          { //waiting for PINDA probe to cool, if this takes more then time expected, temp. cal. fails
        ...
        if (start_temp < 35)
          start_temp = 35;
        if (start_temp < current_temperature_pinda)
          start_temp += 5;
	...
	*/
    ...
    case_G80:
    {
      ...
      while (mesh_point != nMeasPoints * nMeasPoints)
      {
        ...
        if ((ix == 0) && (iy == 0))
          ...
          current_position[Z_AXIS] += 2.f / nMeasPoints; //use relative movement from Z coordinate where PINDa triggered on previous point. This makes calibration faster.
        ...
        const float Z_CALIBRATION_THRESHOLD = 2.f;
        if (!find_bed_induction_sensor_point_z((has_z && mesh_point > 0) ? z0 - Z_CALIBRATION_THRESHOLD : -10.f, nProbeRetry))
        { //if we have data from z calibration max allowed difference is 1mm for each point, if we dont have data max difference is 10mm from initial point
          printf_P(_T(MSG_BED_LEVELING_FAILED_POINT_LOW));
          break;
        }
        if (init_z_bckp - current_position[Z_AXIS] < 0.1f)
        { //broken cable or initial Z coordinate too low. Go to MESH_HOME_Z_SEARCH and repeat last step (z-probe) again to distinguish between these two cases.
        ...
        { //if we have data from z calibration, max. allowed difference is 1mm for each point
    ...
    // ~ Z-homing (can not be used "G28", because X & Y-homing would have been done before (Z-homing))
...
#ifndef PINDA_THERMISTOR
      if (temp_cal_active == true && calibration_status_pinda() == true)
      ...
      for (uint8_t i = 0; i < 4; ++i)
      {
        ...
        {
          unsigned char *addr = (i < 2) ? ((i == 0) ? (unsigned char *)EEPROM_BED_CORRECTION_LEFT : (unsigned char *)EEPROM_BED_CORRECTION_RIGHT) : ((i == 2) ? (unsigned char *)EEPROM_BED_CORRECTION_FRONT : (unsigned char *)EEPROM_BED_CORRECTION_REAR);
...
#error "You must have a Z_MIN endstop in order to enable calculation of Z-Probe repeatability."
   ...
   */
      case 73: //M73 show percent done and time remaining
        if (code_seen('P'))
          print_percent_done_normal = code_value();
        if (code_seen('R'))
          print_time_remaining_normal = code_value();
        if (code_seen('Q'))
          print_percent_done_silent = code_value();
        if (code_seen('S'))
...
// a print without suicide...
    ...
    */
      ...
      {
        if (code_seen('S'))
          cs.minimumfeedrate = code_value();
        if (code_seen('T'))
          cs.mintravelfeedrate = code_value();
        if (code_seen('B'))
          cs.minsegmenttime = code_value();
        if (code_seen('X'))
          cs.max_jerk[X_AXIS] = cs.max_jerk[Y_AXIS] = code_value();
        if (code_seen('Y'))
          cs.max_jerk[Y_AXIS] = code_value();
        if (code_seen('Z'))
          cs.max_jerk[Z_AXIS] = code_value();
        if (code_seen('E'))
        {
          float e = code_value();
#ifndef LA_NOCOMPAT
        ...
        if (cs.max_jerk[X_AXIS] > DEFAULT_XJERK)
          cs.max_jerk[X_AXIS] = DEFAULT_XJERK;
        if (cs.max_jerk[Y_AXIS] > DEFAULT_YJERK)
          cs.max_jerk[Y_AXIS] = DEFAULT_YJERK;
      }
      break;
    ...
    */
      case 301:
      {
        if (code_seen('P'))
          cs.Kp = code_value();
        if (code_seen('I'))
          cs.Ki = scalePID_i(code_value());
        if (code_seen('D'))
          cs.Kd = scalePID_d(code_value());
    ...
    */
      case 304:
      {
        if (code_seen('P'))
          cs.bedKp = code_value();
        if (code_seen('I'))
          cs.bedKi = scalePID_i(code_value());
        if (code_seen('D'))
          cs.bedKd = scalePID_d(code_value());
    ...
    */
      case 861:
        if (code_seen('?'))
        { // ? - Print out current EEPROM offset values
          ...
          {
            if (i > 0)
              EEPROM_read_B(EEPROM_PROBE_TEMP_SHIFT + (i - 1) * 2, &usteps);
        ...
        { // Sxxx Iyyy - Set compensation ustep value S for compensation table index I
...
*/
    ...
    */
      case 907:
      {
        ...
        for (int i = 0; i < NUM_AXIS; i++)
          if (code_seen(axis_codes[i]))
            st_current_set(i, code_value());
        if (code_seen('B'))
          st_current_set(4, code_value());
        if (code_seen('S'))
          for (int i = 0; i <= 4; i++)
            st_current_set(i, code_value());
#endif
        ...
        if (code_seen('X'))
          st_current_set(0, code_value());
#endif
#ifdef MOTOR_CURRENT_PWM_Z_PIN
        if (code_seen('Z'))
          st_current_set(1, code_value());
#endif
#ifdef MOTOR_CURRENT_PWM_E_PIN
        if (code_seen('E'))
          st_current_set(2, code_value());
#endif
#endif //TMC2130
      }
      break;
    ...
    */
      ...
      {
        if (code_seen('X'))
          tmc2130_set_current_h(0, code_value());
        if (code_seen('Y'))
          tmc2130_set_current_h(1, code_value());
        if (code_seen('Z'))
          tmc2130_set_current_h(2, code_value());
        if (code_seen('E'))
          tmc2130_set_current_h(3, code_value());
      }
      break;
    ...
    */
      ...
      {
        if (code_seen('X'))
          tmc2130_set_current_r(0, code_value());
        if (code_seen('Y'))
          tmc2130_set_current_r(1, code_value());
        if (code_seen('Z'))
          tmc2130_set_current_r(2, code_value());
        if (code_seen('E'))
          tmc2130_set_current_r(3, code_value());
      }
      break;
    ...
    */
      ...
      {
        if (code_seen('X'))
          tmc2130_sg_thr[X_AXIS] = code_value();
        if (code_seen('Y'))
          tmc2130_sg_thr[Y_AXIS] = code_value();
        if (code_seen('Z'))
          tmc2130_sg_thr[Z_AXIS] = code_value();
        if (code_seen('E'))
          tmc2130_sg_thr[E_AXIS] = code_value();
    ...
    */
      ...
      {
        if (code_seen('X'))
          tmc2130_set_pwm_ampl(0, code_value());
        if (code_seen('Y'))
          tmc2130_set_pwm_ampl(1, code_value());
        if (code_seen('Z'))
          tmc2130_set_pwm_ampl(2, code_value());
        if (code_seen('E'))
          tmc2130_set_pwm_ampl(3, code_value());
      }
      break;
    ...
    */
      ...
      {
        if (code_seen('X'))
          tmc2130_set_pwm_grad(0, code_value());
        if (code_seen('Y'))
          tmc2130_set_pwm_grad(1, code_value());
        if (code_seen('Z'))
          tmc2130_set_pwm_grad(2, code_value());
        if (code_seen('E'))
          tmc2130_set_pwm_grad(3, code_value());
      }
      break;
...
void get_coordinates()
{
  ...
  if (code_seen('F'))
  {
    ...
    if (tmc2130_mode == TMC2130_MODE_SILENT)
      if (next_feedrate > MAX_SILENT_FEEDRATE)
        next_feedrate = MAX_SILENT_FEEDRATE;
...
void clamp_to_software_endstops(float target[3])
{
  ...
  if (min_software_endstops)
  {
    ...
    if (cs.add_homing[Z_AXIS] < 0)
      negative_z_offset = negative_z_offset + cs.add_homing[Z_AXIS];
#endif
...
//-//		if (mcode_in_progress != 600) //M600 not in progress
    ...
    if ((mcode_in_progress != 600) && (eFilamentAction != FilamentAction::AutoLoad) && (!bInhibitFlag)) //M600 not in progress, preHeat @ autoLoad menu not active, Support::ExtruderInfo/SensorInfo menu not active
    {
      if (!moves_planned() && !IS_SD_PRINTING && !is_usb_printing && (lcd_commands_type != LcdCommands::Layer1Cal) && !eeprom_read_byte((uint8_t *)EEPROM_WIZARD_ACTIVE))
      {
        ...
        if (current_voltage_raw_IR > maxVolt)
          maxVolt = current_voltage_raw_IR;
        if (current_voltage_raw_IR < minVolt)
          minVolt = current_voltage_raw_IR;
...
void check_babystep()
{
  int babystep_z = eeprom_read_word(reinterpret_cast<uint16_t *>(&(EEPROM_Sheets_base->s[(eeprom_read_byte(&(EEPROM_Sheets_base->active_sheet)))].z_offset)));
  ...
  if ((babystep_z < Z_BABYSTEP_MIN) || (babystep_z > Z_BABYSTEP_MAX))
  {
    ...
    eeprom_write_word(reinterpret_cast<uint16_t *>(&(EEPROM_Sheets_base->s[(eeprom_read_byte(&(EEPROM_Sheets_base->active_sheet)))].z_offset)),
                      babystep_z);
...
static void temp_compensation_start()
{
  ...
  plan_buffer_line_curposXYZE(3000 / 60, active_extruder);
  st_synchronize();
  while (fabs(degBed() - target_temperature_bed) > 1)
    delay_keep_alive(1000);
  ...
  for (int i = 0; i < PINDA_HEAT_T; i++)
  {
    ...
    if (custom_message_state == 99 || custom_message_state == 9)
      lcd_update(2); //force whole display redraw if number of digits changed
    else
      lcd_update(1);
  }
  custom_message_type = CustomMsg::Status;
...
float temp_comp_interpolation(float inp_temperature)
{
  ...
  {
    if (i > 0)
      EEPROM_read_B(EEPROM_PROBE_TEMP_SHIFT + (i - 1) * 2, &shift[i]); //read shift in steps from EEPROM
...
void uvlo_()
{
  ...
  current_position[Z_AXIS] += float(1024 - z_microsteps) / (z_res * cs.axis_steps_per_unit[Z_AXIS]) + UVLO_Z_AXIS_SHIFT;
  plan_buffer_line_curposXYZE(homing_feedrate[Z_AXIS] / 60, active_extruder);
  st_synchronize();
  poweroff_z();
...
void uvlo_tiny()
{
  ...
  if (abs(current_position[Z_AXIS] - eeprom_read_float((float *)(EEPROM_UVLO_TINY_CURRENT_POSITION_Z))) >= 1.f / cs.axis_steps_per_unit[Z_AXIS])
  {
    ...
    uint16_t z_res = tmc2130_get_res(Z_AXIS);
    uint16_t z_microsteps = tmc2130_rd_MSCNT(Z_AXIS);
    current_position[Z_AXIS] += float(1024 - z_microsteps) / (z_res * cs.axis_steps_per_unit[Z_AXIS]) + UVLO_TINY_Z_AXIS_SHIFT;
    plan_buffer_line_curposXYZE(homing_feedrate[Z_AXIS] / 60, active_extruder);
    st_synchronize();
    poweroff_z();
...
ISR(INT7_vect)
{
...
#ifdef FAN_SOFT_PWM
  if (!fan_measuring || (fanSpeedSoftPwm < MIN_PRINT_FAN_SPEED))
    return;
#else  //FAN_SOFT_PWM
  ...
  if ((1 << 6) & EICRB)
  { //interrupt was triggered by rising edge
    t_fan_rising_edge = millis_nc();
  }
  else
  { //interrupt was triggered by falling edge
    if ((millis_nc() - t_fan_rising_edge) >= FAN_PULSE_WIDTH_LIMIT)
    {                           //this pulse was from sensor and not from pwm
...
ISR(INT4_vect)
{
  ...
  if (PRINTER_ACTIVE && (!(eeprom_read_byte((uint8_t *)EEPROM_UVLO))))
    uvlo_();
...
void stop_and_save_print_to_ram(float z_move, float e_move)
{
  ...
  {                            //reuse saved_sdpos for storing line number
...
void restore_print_from_ram_and_continue(float e_move)
{
  ...
  if ((fan_check_error != EFCE_OK) && (fan_check_error != EFCE_FIXED))
    return;
  if (fan_check_error == EFCE_FIXED)
    fan_check_error = EFCE_OK; //reenable serial stream processing if printing from usb
#endif
...
uint16_t print_time_remaining()
{
  ...
  if (SilentModeMenu == SILENT_MODE_OFF)
    ...
    print_t = print_time_remaining_normal;
#endif //TMC2130
  if ((print_t != PRINT_TIME_REMAINING_INIT) && (feedmultiply != 0))
