...
int8_t xyzcal_meassure_pinda_hysterezis(int16_t min_z, int16_t max_z, uint16_t delay_us, uint8_t samples)
{
	...
	if (!_PINDA)
	{
		...
		{
			dn = _Z;
			if (!xyzcal_lineXYZ_to(_X, _Y, min_z, delay_us, 1))
				break;
			dn = dn - _Z;
			up = _Z;
			if (!xyzcal_lineXYZ_to(_X, _Y, max_z, delay_us, -1))
				break;
			up = _Z - up;
			DBG(_n("%d. up=%d dn=%d\n"), sample, up, dn);
			sum_up += up;
...
int16_t xyzcal_match_pattern_12x12_in_32x32(uint16_t *pattern, uint8_t *pixels, uint8_t c, uint8_t r)
{
	...
	for (uint8_t i = 0; i < 12; i++)
		...
		{
			if (((i == 0) || (i == 11)) && ((j < 2) || (j >= 10)))
				continue; //skip corners
			if (((j == 0) || (j == 11)) && ((i < 2) || (i >= 10)))
				continue;
...
bool xyzcal_scan_and_process(void)
{
	...
	xyzcal_scan_pixels_32x32(x, y, z - 372, 2400, 200, pixels);
	...
	if (xyzcal_searchZ())
	{
		...
		if (xyzcal_scan_and_process())
		{
			if (xyzcal_find_point_center2(500))
			{
				...
				for (n = 0; n < 4; n++)
				{
					if (!xyzcal_find_point_center2(1000))
						break;
					x_avg += _X;
					y_avg += _Y;
				}
